...
**Per-message actions (below each chat bubble)**

- Under **every** chat bubble (both the user's and the assistant's), show a row of small, subtle icon buttons **aligned to the right-hand side, just below the bubble** - quiet by default, more visible on hover/focus - for these actions. Each has an accessible label and tooltip:
  - **Copy** - copy that message's text to the clipboard (show a brief "Copied" confirmation).
  - **Revert to here** - return the chat to this message by deleting every message after it (calls `POST /api/chats/:id/revert`). Because it is destructive, confirm first; then replace the visible messages with the trimmed history.
  - **Fork** - create a new chat that includes all history up to and including this message (calls `POST /api/chats/:id/fork`), then switch to the new chat. The original chat is left intact.
...
- Use **one toggle switch** that is both the status and the action - do not show a separate status pill plus an action button (that duplicates the same information). Poll `GET /api/models/runtime` periodically or refresh on focus to keep the switch state accurate. The switch position and color carry the state; a short label next to it spells it out. It has three visual states:
...
- Include at least these sections (headings shown as they should appear):
  ...
  - **Start and stop the app** (list this **second, right after Tip the creator**) - **lead with the one-click launcher**: the easiest way to start is to **double-click `Free AI Forever.command` (macOS) or `Free AI Forever.bat` (Windows)** in the app's folder - no typing. State clearly: (1) double-click that launcher file (on macOS the first time only, right-click → Open to clear Gatekeeper); (2) a small terminal window opens, the launcher **starts the server for you**, and the app appears in its own window - keep that terminal window open the whole time; (3) to stop, close that terminal window (or press Ctrl+C / Control + C); chats are saved. Then offer the **terminal alternative** for anyone who prefers it, in two **copy boxes**: a `cd path/to/Free-AI-Forever` template (go into the app's folder first - the folder that contains `package.json`) and `npm run dev`. Include a short **troubleshooting note** (visually distinct callout): if double-clicking does nothing, confirm Node.js is installed (nodejs.org); if using the terminal you see `npm error … Could not read package.json … ENOENT`, the command was run outside the app's folder, so do the `cd` step into the app's folder first, then run `npm run dev` again.
  - **Install it for a Dock or taskbar icon** (list this **third, right after Start and stop the app**) - explain that once the app is open in its window, the user can install it so it gets its own icon: in the app window's browser menu choose "Install Free AI Forever" (or the install icon in the address bar) to pin it to the Dock/Launchpad (macOS) or taskbar/Start menu (Windows). Note that the installed icon is a shortcut - the launcher still has to be running (it starts the local server) for the installed app to load.
...
- Small action icons under each chat bubble: **Copy** the message, **Revert** the chat to that message (deletes later messages, with confirmation), and **Fork** a new chat that keeps the history up to that message. Disabled while streaming and for unsaved messages.
...
- A single toggle switch that is both status and action: Off (grey, RAM freed), Starting… (amber, spinner while loading into RAM), and On (green, model loaded). No separate status pill - the switch carries the state.
...
- **`launch.mjs`** (project root, plain Node, no extra dependencies): on run it (1) installs dependencies if any `node_modules` are missing, (2) builds the app if `backend/dist` or `frontend/dist` is missing, (3) starts the single-port server as a child process, (4) polls `GET /api/health` until it answers, then (5) opens the app in a **standalone app window**. For the window, launch Chrome/Edge (or Brave/Chromium) with `--app=http://127.0.0.1:<port>/` so there are no tabs or address bar and it gets its own Dock/taskbar entry; if no Chromium-family browser is found, fall back to opening the default browser at the URL. Handle SIGINT/SIGTERM by stopping the child server. On macOS launch the browser binary directly (e.g. `/Applications/Google Chrome.app/Contents/MacOS/Google Chrome`) so `--app` is honored even when the browser is already running; on Windows use `start "" chrome --app=...` then `msedge`, then the default browser.
...
### Installable app icon (PWA)

Make the served app installable so it can live in the Dock/taskbar/Start menu with its own icon and open in its own window:

- Add a **web app manifest** (`frontend/public/manifest.webmanifest`) with `name` "Free AI Forever", `short_name`, `start_url` `/`, `scope` `/`, `display` `standalone`, a `background_color`, a `theme_color` matching the accent (`#2680EB`), and at least one icon. Link it from `index.html` (`<link rel="manifest" ...>`) along with `theme-color`, `icon`, and `apple-touch-icon`.
- Provide an app **icon** (`frontend/public/icon.svg`, accent-blue mark). Because these live in `public/`, they are copied to `frontend/dist` and served by the single-port server at the site root (verify the manifest is served as `application/manifest+json` and the icon as `image/svg+xml`).
- With the manifest present, Chrome/Edge offer "Install Free AI Forever" from the browser menu / address-bar install icon, giving a real installed-app icon. (A service worker is optional and intentionally omitted to avoid serving a stale cached build; the app needs the local server running anyway.)
...
EASIEST WAY (no typing) - just double-click
  ...
  3. A small Terminal window opens and the app opens in its own window. The first
  ...
  Optional: in the app window's browser menu choose "Install Free AI Forever" to
  pin it to your Dock with its own icon and open it like any other app.
...
EASIEST WAY (no typing) - just double-click
  ...
  3. A small window opens and the app opens in its own window. The first launch
  ...
  Optional: in the app window's browser menu choose "Install Free AI Forever" to
  pin it to your taskbar/Start menu with its own icon and open it like any app.
